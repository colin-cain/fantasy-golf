...
export default async function RootLayout({
  ...
  let tickerPicks: { member: string; golfer: string; position: string | null; total: string | null; thru: string | null }[] = []
  if (live) {
    ...
    if (picks?.length) {
      ...
      const { data: cache } = await supabase
        ...
        .select('golfer_name, position, total, thru')
        .in('golfer_name', golferNames)

      const cacheMap = Object.fromEntries(
        (cache ?? []).map((r: { golfer_name: string; position: string; total: string; thru: string }) => [r.golfer_name, r])
      )
